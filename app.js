...
//   generated: {}, // сюда заносим pid-ы процессов, и отправленные
//                  // &&обработтанныt данные для каждого из pid
...
function storeWorker(pid, opt = {}) {
  ...
  const alive = (typeof opt.alive === 'boolean') ? opt.alive : !!worker;
  workers[pid] = {
    pid,
...
function start() {
  ...
  ws.on('worker.fork', (reply) => {
    ...
    // дочерний процесс готов (запустил IPC сервер, подключился к мастеру
    // а мастер подключился к дочернему процессу)
