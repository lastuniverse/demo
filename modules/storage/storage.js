...
/**
 * Модуль реализует автоматическое сохранение данных при выходе из программы (завершении процесса)
 * и их восстановление при повторном запуске
 *
 ...
 * - [L] Принцип подстановки Лисков (достигается за счет того, что все реализации
 *       наследуются от прототипа DummyStorage и следуют соглашению/рекомендациям
 *       по переопределению методов класса DummyStorage)
 * - [I] Принцип разделения интерфейса (тут очевидно...)
 * - [D] Принцип инверсии зависимостей (Достигается за счет того, что класс DummyStorage
 *       по сути является абстракцией, не зависящей от реализаций)
 *
 * @example
...
/**
 * Загружает класс-реализацию механизмов фонового сохранения
 * и загрузки данных и создает объект от класса реализации
 *
 * @class      Storage (name)
 * @param      {array}                  options  Набор произвольных параметров,
 *                                               передаваемых конструктору реализации
 *                                               Каждая реализация обладает
 *                                               своим набором параметров.
 * @return     {StorageImplementation}  объект класса-реализации
 */
function Storage(...options) {
  try {
    const StorageImplementation = require(`./implementations/${defaults.type}-storage.js`);
