...
class FileStorage extends Storage {
   ...
   * @param      {object|array}  data      пустой объект необходимого типа данных
   *                                       (массив или объект)
   * @param      {boolean}       humanize  (true|false) сохранять данные в развернутом виде
   * @return     {object|array}  сохранаемое хранилище данных. Если данные уже были
   * сохранены, то будет содержать их восстановленную копию. Во внешнем коде использующем
   * данные нельзя работать с копией данных, так как данные из копий не будут сохраняться
   */
  ...
  /**
   * сохраняет в файлах все именованые коллекции данных (необходимо использовать только
   * синхронные механизмы) выполняется автоматически при завершениее процесса (программы)
   */
  storeAll() {
