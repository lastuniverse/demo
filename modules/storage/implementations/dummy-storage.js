 ...
 * насследоваться от данного класса, могут переопределять методы data:create(name, data),
 * store(name), storeAll() и restore(name)
 ...
 *
 * В реализациях, для успешного выполнения сохранения данных (в методах store(name), storeAll())
...
const signalEvents = [
  ...
  // посредством CTRL+C (можно перенастроить). Не генерируется, когда включен «сырой» режим
  // терминала. Если один этот сигнал имеет установленный слушатель, его поведение по умолчанию
  // будет удалено (Node.js больше не будет завершаться).
  {
    signal: 'SIGINT',
    exit: true,
  },
  ...
  // также при подобных условиях (см. signal(7)). Он может иметь установленный слушатель, однако,
  // Node.js будет закрыт Windows спустя 10 секунд. На отличных от Windows платформах поведение
  // SIGHUP по умолчанию заключается в принудительном завершении Node.js,
  ...
  // SIGBREAK работает на Windows, когда нажато <Ctrl> + <Break>, на не-Windows платформах
  // его можно слушать, но нельзя отправить или сгенерировать.
  {
    signal: 'SIGBREAK',
    exit: true,
  },
