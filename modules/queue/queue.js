/**
 * Модуль реализует очереди (хранилища с соблюдением порядка очередности) с опциональной
 * возможностью автоматически сохранять данные очереди при выходе из программы
 * (завершении процесса) и восстанавливать их при последующих запусках
 *
 *
 * @example
...
class Queue {
   ...
   *
   * @param      {string}   name      наименование очереди. Если задано, то будет создаваться
   *                                  автоматически сохраняемое и восстанавливаемое хранилище
   */
  constructor(name) {
   ...
   *
   * @param      {<type>}  data      любые данные (если очередь с автосохранением, то любые
   *                                 данные приводимые к строке с помощью JSON.stringify(data))
   * @param      {<type>}  priority  порядковый номер, по которому определяется приоритет данных
   *                                 в очереди, если не задан, то присваивается автоматически
   */
